...
export const getChannelSubscribers = async (username) => {
  try {
    const { data } = await API.get(`/subscription/c/${username}`);

    return data?.data || [];
  } catch (error) {
    console.log("Failed to fetch channel subscribers:",error);
    throw error?.response?.data?.error;
  }
};
